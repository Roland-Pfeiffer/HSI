...
spectra = pyperspectral.Spectra(files[0].intensities, files[0].wlv, files[0].material)
for spec in files[1:]:
...
descs = [pyperspectral.TriangleDescriptor(d[0], d[1], d[2], material) for d in descs]

# Create desc set
for i, desc in enumerate(descs):
    if i == 0:
        set = pyperspectral.DescriptorSet(desc)
    else:
